...
function tier3(rng: Rng): Problem {
  ...
  if (!useLog) {
    ...
    return {
      ...
      solution: [
        ...
        { text: 'Evaluate at the new limits:', tex: `= ${ratCoefPrefix(half)}\\left[e^{u}\\right]_0^{${U2}} = ${ratCoefPrefix(half)}\\left(e^{${U2}}-1\\right) = ${value}` },
      ],
  ...
  return {
    ...
    solution: [
      ...
      { text: 'Evaluate at the new limits:', tex: `= ${ratCoefPrefix(half)}\\left[\\ln(u)\\right]_{${k}}^{${upper}} = ${ratCoefPrefix(half)}\\ln\\left(\\frac{${upper}}{${k}}\\right) = ${ratCoefPrefix(half)}\\ln(2) = ${value}` },
    ],
    hints: HINTS_DEFINITE,
