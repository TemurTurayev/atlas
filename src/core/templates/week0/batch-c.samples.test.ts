...
describe('geometry', () => {
  it('tier 2: circle area/circumference matches r and pi independently', () => {
    for (let seed = 1; seed <= 40; seed += 1) {
      ...
      const match = p.statement.match(/radius \$(\d+)\$ cm\. Find its (area|circumference)/)
      if (!match) throw new Error(`unexpected statement: ${p.statement}`)
...
describe('pythagoras', () => {
  it('tier 1: hypotenuse satisfies a^2+b^2=c^2', () => {
    for (let seed = 1; seed <= 40; seed += 1) {
      ...
      if (p.answer.kind !== 'number') throw new Error('expected number')
      const match = p.statement.match(/legs \$(\d+)\$ and \$(\d+)\$/)
      if (!match) throw new Error(`unexpected statement: ${p.statement}`)
...
describe('lines_2d', () => {
  it('tier 1: slope matches (y2-y1)/(x2-x1) independently', () => {
    for (let seed = 1; seed <= 40; seed += 1) {
      ...
      if (p.answer.kind !== 'number') throw new Error('expected number')
      const match = p.statement.match(/through \$\((-?\d+), (-?\d+)\)\$ and \$\((-?\d+), (-?\d+)\)\$/)
      if (!match) throw new Error(`unexpected statement: ${p.statement}`)
...
describe('graphs_basic', () => {
  it('tier 1: the vertex satisfies the parabola equation', () => {
    for (let seed = 1; seed <= 40; seed += 1) {
      ...
      const h = Number(hStr)
      const k = Number(kStr)
      const expr = lastMath(p.statement).replace('y =', '').trim()
...
describe('trig_triangle', () => {
  it('tier 1: side matches hypotenuse times sin/cos independently', () => {
    for (let seed = 1; seed <= 40; seed += 1) {
      ...
      if (p.answer.kind !== 'number') throw new Error('expected number')
      const match = p.statement.match(/hypotenuse \$(\d+)\$ and an acute angle of \$(\d+)/)
      if (!match) throw new Error(`unexpected statement: ${p.statement}`)
      const hyp = Number(match[1])
      const angle = Number(match[2])
      const isOpposite = p.statement.includes('the side opposite')
...
describe('radians', () => {
  it('tier 1: degrees to radians matches deg*pi/180 independently', () => {
    for (let seed = 1; seed <= 40; seed += 1) {
      ...
      if (p.answer.kind !== 'number') throw new Error('expected number')
      const match = p.statement.match(/Convert \$(\d+)/)
      if (!match) throw new Error(`unexpected statement: ${p.statement}`)
