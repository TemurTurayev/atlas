...
function tier1(rng: Rng): Problem {
  if (rng.chance(0.5)) {
    ...
    return build(`\\sqrt{${n * n}}`, String(n), [{ ru: `Ищем число, квадрат которого равен $${n * n}$, ведь $${n}^{2} = ${n * n}$:`, tex: `\\sqrt{${n * n}} = ${n}` }], HINTS_BASIC)
  }
  const n = rng.int(2, 6)
  return build(`\\sqrt[3]{${n ** 3}}`, String(n), [{ ru: `Ищем число, куб которого равен $${n ** 3}$, ведь $${n}^{3} = ${n ** 3}$:`, tex: `\\sqrt[3]{${n ** 3}} = ${n}` }], HINTS_BASIC)
}
...
function tier2(rng: Rng): Problem {
  ...
  return build(equation, String(value), [
    { ru: `Показатель $\\frac{${m}}{${d}}$ значит: корень степени $${d}$, затем степень $${m}$:`, tex: `${base}^{\\frac{${m}}{${d}}} = \\left(\\sqrt[${d}]{${base}}\\right)^{${m}}` },
    { ru: `Корень степени $${d}$ из $${base}$ равен $${k}$, так как $${k}^{${d}} = ${base}$:`, tex: `\\sqrt[${d}]{${base}} = ${k}` },
    { ru: 'Возводим в степень:', tex: `${k}^{${m}} = ${value}` },
  ], HINTS_FRAC)
}
...
function rootsProduct(rng: Rng): Problem {
  ...
  return build(`\\sqrt{${a}} \\cdot \\sqrt{${b}}`, String(value), [
    { ru: 'Объединяем под одним корнем:', tex: `\\sqrt{${a}} \\cdot \\sqrt{${b}} = \\sqrt{${a} \\cdot ${b}} = \\sqrt{${a * b}}` },
    { ru: `Под корнем — точный квадрат $${value}^{2}$:`, tex: `\\sqrt{${a * b}} = \\sqrt{${value}^{2}} = ${value}` },
  ], HINTS_T3)
}
...
function rootPower(rng: Rng): Problem {
  ...
  return build(`\\left(\\sqrt{${n}}\\right)^{4}`, String(value), [
    { ru: 'Разбиваем четвёртую степень на квадрат квадрата:', tex: `\\left(\\sqrt{${n}}\\right)^{4} = \\left(\\left(\\sqrt{${n}}\\right)^{2}\\right)^{2} = ${n}^{2}` },
    { ru: 'Вычисляем:', tex: `${n}^{2} = ${value}` },
  ], HINTS_T3)
}
...
function rationalizeDenominator(rng: Rng): Problem {
  ...
  return build(`\\frac{1}{\\sqrt{${n}}}`, value, [
    { ru: `Умножаем числитель и знаменатель на $\\sqrt{${n}}$:`, tex: `\\frac{1}{\\sqrt{${n}}} = \\frac{1\\cdot\\sqrt{${n}}}{\\sqrt{${n}}\\cdot\\sqrt{${n}}} = \\frac{\\sqrt{${n}}}{${n}}` },
  ], HINTS_T3)
}
